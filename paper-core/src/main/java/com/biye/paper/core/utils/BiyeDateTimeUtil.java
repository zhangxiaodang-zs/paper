...
public class BiyeDateTimeUtil {
    ...
    public static List<String> getIntervalTimeList(String start, String end, int interval) throws Exception {
        ...
        Date startDate = BiyeDateTimeUtil.convertString2Date("HH:mm", start);
        Date endDate = BiyeDateTimeUtil.convertString2Date("HH:mm", end);

        List<String> list = new ArrayList<>();

        while (startDate.getTime() <= endDate.getTime()) {
            list.add(BiyeDateTimeUtil.convertDate2String("HHmm", startDate));
            Calendar calendar = Calendar.getInstance();
            calendar.setTime(startDate);
            calendar.add(Calendar.MINUTE, interval);
            if (calendar.getTime().getTime() > endDate.getTime()) {
                if (!startDate.equals(endDate)) {
                    list.add(BiyeDateTimeUtil.convertDate2String("HHmm", endDate));
                }
